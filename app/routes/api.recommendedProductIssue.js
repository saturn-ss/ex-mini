...
export const loader = async ({ request }) => {
  // The authenticate.admin method returns a CROS method to automatically wrap responses so that extensions, which are hosted on textensions, which are hosted on extensions.shopifycdn.com, can access this route.

  const { cors } = await authenticate.admin(request);

  const productIssues = [
    { title: "Too big", description: "The product was too big."},
    { title: "Too small", description: "The proudct was too small." },
    {
      title: "Just right",
